...
function renderAccounts() {
    ...
    if (currAccount) {
        ...
        accountsHTML.innerHTML = "<p>Please select from the accounts below to log in</p>"
    }
  }
  ...
  .then(function (jsonObject) {
      ...
      function checkpassword(event){
          if (document.getElementById("login").accountPass.value == accounts[getLocalStorage("tempaccountnum")].Password){
...
function displayAccount(account){
    ...
    function login(accountName){
        document.getElementById("account-header").textContent = accountName.currentTarget.name + " Login";
        ...
        setTimeout(() => {  body.addEventListener("click", toggleLogin, false)}, 100);
    ...
    function toggleLogin(){
        ...
        document.getElementById("login-bg").classList.toggle("toggleTransparent");
        document.getElementById("login-bg").removeEventListener("click", toggleLogin);
    }
